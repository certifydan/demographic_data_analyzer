...
def calculate_demographic_data(print_data=True):
    ...
    # count total number of people by country, count people with salary > 50k calculate percentage
    people_by_country = df['native-country'].value_counts()
    rich_ppl_by_country = df.loc[df['salary'] == '>50K', 'native-country'].value_counts()
    richest_countries = rich_ppl_by_country / people_by_country
    
    highest_earning_country = richest_countries.nlargest().index[0]
    highest_earning_country_percentage = round(richest_countries.max()*100, 1)
